...
import { LinkedInToken, LinkedInProfile, PostAnalytics, OAuthTokenResponse } from '../types.js';
...
export async function getValidToken(userId: string): Promise<string> {
  ...
  if (!token) {
    throw new Error(`No token found for user ${userId}. Authenticate at /auth/linkedin?user_id=${userId}`);
  }
  if (isTokenExpired(token) && token.refresh_token) {
    ...
    await storeToken(userId, updatedToken);
...
async function linkedInRequest<T>(
  ...
  if (!response.ok) {
    let errMsg = `LinkedIn API error: ${response.status}`;
    try {
...
export async function getPostAnalytics(
  ...
  return {
    ...
    impressionCount: 0,
    clickCount: 0,
    likeCount: reactions.paging?.total ?? 0,
    commentCount: comments.paging?.total ?? 0,
    shareCount: 0,
    engagementRate: 0,
  };
}
